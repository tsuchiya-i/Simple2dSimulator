...
try:
    agent.load_weights('test_{}_weights.h5f'.format(ENV_NAME))
...
agent.save_weights('test_{}_weights.h5f'.format(ENV_NAME), overwrite=True)
...
try:
    load = np.loadtxt('./Simple2dSimulator.csv')
    print("find csvfile")
    csvnp = np.concatenate([load, csvnp])
except:
    print("not found csvfile")

np.savetxt('Simple2dSimulator.csv',csvnp)
